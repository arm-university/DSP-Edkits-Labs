/* Includes ------------------------------------------------------------------*/
#include "stm32f7_square_intr.h"

#define SOURCE_FILE_NAME "stm32f7_square_intr.c"

/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
/* Audio parameters */
#define AUDIO_FREQ            8000 
#define LOOPLENGTH 					  256	
...
int16_t square_table[LOOPLENGTH] = {
  ...
  -10000, -10000, -10000, -10000,
  -10000, -10000, -10000, -10000};
int16_t square_ptr = 0;  // pointer into lookup table
...
{	
    BSP_LED_Toggle(LED1);
    PlayComplete = 1;
    buffer[buf_ptr] = square_table[square_ptr];
...
int main(void)
{
  ...
  SystemClock_Config();
	...
	BSP_LED_Init(LED1);
	GPIOI1_Init();
	plotSamples(square_table, LOOPLENGTH, 256);
	
	if (BSP_AUDIO_OUT_Init(OUTPUT_DEVICE_HEADPHONE, 50, AUDIO_FREQ) != AUDIO_OK) {
			Error_Handler();
	}
	
	if (BSP_AUDIO_OUT_Play((uint16_t*)square_table, LOOPLENGTH * sizeof(int16_t)) != AUDIO_OK) {
			Error_Handler();
	}
  /* Infinite loop */
  while (1)
